...
async def get_user_input_for_open_answer(prev_answer: str) -> str:
    """Get user input asynchronously."""
    while True:
        input_str = await async_input("回答を入力してください: ")
        return prev_answer + input_str.strip()
    
async def test_check_open_answer(question: str, prev_answer: str = ""):
    answer = await get_user_input_for_open_answer(prev_answer)
    is_answer_done, answer_label, observed_prob_of_yes = await check_open_answer(question, answer)
    
    if is_answer_done:
        return observed_prob_of_yes
    else:
        return await test_check_open_answer(question, answer)
...
async def test_uot_full_cycle():
    ...
    for i in range(5):
        ...
        # User input
        p_y = await test_check_open_answer(question)
