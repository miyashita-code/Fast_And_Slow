...
from demo_module.sample_states import get_init_state_walk_preparation
...
class LinearConversationController:
    ...
    def get_init_state(self) -> List[State]:
        ...
        return get_init_state_walk_preparation()
    ...
    async def send_next_message(self):
        ...
        if current_state:
            ...
            await self.direct_prompting_func(
                ...
                f"\n次の内容: {message['next']}***[注意] これを参照した内容を話す前に適切にgo_nextなどの関数をよんでください！！***"
