...
from .custom_congif import MAX_TOKEN_WINDOW
...
class AutoGPTPrompt(BaseChatPromptTemplate, BaseModel):  # type: ignore[misc]
    ...
    send_token_limit: int = MAX_TOKEN_WINDOW
    ...
    def format_messages(self, **kwargs: Any) -> List[BaseMessage]:
        ...
        dialogs_prompt_content = (
            "This is the current dialog datas"
            "between the endpoint(interface) dialog agent &"
            "the user who is supported by the interface agent.\n"
            "From this dialog datas, you can make decision.\n:"
        )

        dialogs_prompt = SystemMessage(content=f"{dialogs_prompt_content}")
        
        used_tokens = self.token_counter(cast(str, base_prompt.content)) \
            + self.token_counter(cast(str, time_prompt.content)) \
            + self.token_counter(cast(str, dialogs_prompt.content))
